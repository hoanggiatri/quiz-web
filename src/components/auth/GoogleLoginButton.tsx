...
export default function GoogleLoginButton({
  ...
  // Fallback button for when Google script is not available
  const handleFallbackClick = () => {
    if (!import.meta.env.VITE_GOOGLE_CLIENT_ID) {
      onError(new Error('Google Client ID not configured. Please set VITE_GOOGLE_CLIENT_ID in your environment variables.'));
      return;
    }
    ...
    <div className="w-full">
      ...
      {!window.google && (
        <Button
          type="button"
          variant="outline"
          className="w-full h-12 text-sm font-medium border-2 hover:bg-blue-50 hover:border-blue-300 transition-all duration-200 mt-2"
