...
const mockAssignments: Assignment[] = [
  {
    ...
    description: "Kiểm tra kiến thức về đại số và hình học cơ bản. Bao gồm các bài tập về phương trình bậc nhất, bậc hai và tính chất hình học.",
    type: "quiz",
    status: "assigned",
  ...
  },
  {
    ...
    description: "Xây dựng một ứng dụng web đơn giản sử dụng React và TypeScript. Yêu cầu implement các tính năng CRUD cơ bản.",
    type: "project",
    status: "in_progress",
  ...
  },
  {
    ...
    description: "Bài thi cuối kỳ môn Vật lý đại cương. Bao gồm các chương về cơ học, nhiệt học và điện học.",
    type: "exam",
    status: "graded",
  ...
  },
  {
    ...
    description: "Bài tập về ngữ pháp và từ vựng Unit 5. Tập trung vào thì hiện tại hoàn thành và từ vựng về du lịch.",
    type: "homework",
    status: "overdue",
  ...
  },
  {
    ...
    description: "Viết một bài tiểu luận 2000 từ về vai trò của Hồ Chí Minh trong cuộc kháng chiến chống Pháp.",
    type: "essay",
    status: "submitted",
...
export const assignmentManagementService = {
  ...
  getAssignments: async (params: AssignmentSearchParams = {}): Promise<AssignmentListResponse> => {
    // Simulate API delay
    ...
    // Apply filters
    if (params.filters) {
      const { status, type, subject, difficulty, dueDateRange } = params.filters;

      if (status && status.length > 0) {
        filteredAssignments = filteredAssignments.filter(a => status.includes(a.status));
      }

      if (type && type.length > 0) {
        filteredAssignments = filteredAssignments.filter(a => type.includes(a.type));
      }

      if (subject && subject.length > 0) {
        filteredAssignments = filteredAssignments.filter(a => subject.includes(a.subject));
      }

      if (difficulty && difficulty.length > 0) {
        filteredAssignments = filteredAssignments.filter(a => difficulty.includes(a.difficulty));
      }

      if (dueDateRange) {
        if (dueDateRange.from) {
          filteredAssignments = filteredAssignments.filter(a => 
            new Date(a.dueDate) >= new Date(dueDateRange.from!)
          );
        }
        if (dueDateRange.to) {
          filteredAssignments = filteredAssignments.filter(a => 
            new Date(a.dueDate) <= new Date(dueDateRange.to!)
          );
        }
      }
    }
  ...
  startAssignment: async (id: string): Promise<{ success: boolean; redirectUrl: string }> => {
  ...
  // Nộp assignment
  submitAssignment: async (id: string, answers: any): Promise<AssignmentResult> => {
